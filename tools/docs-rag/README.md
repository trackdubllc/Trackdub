...
A repository entry may carry `exclude`: repo-relative patterns matched with `PurePosixPath.full_match`, used to drop mirrored aggregate pages whose bodies duplicate the leaf docs already in the corpus. A pattern matching nothing fails the refresh, so a renamed or deleted mirror cannot silently stop being excluded.
