...
2. **TensorRT RTX plugin route** — `ExecutionProviderKind.TensorRTRtx` is the standalone ORT EP ABI plugin route on Windows. Trackdub locates a bundle containing `onnxruntime_providers_nv_tensorrt_rtx.dll`, `tensorrt_rtx_1_6.dll`, and `tensorrt_onnxparser_rtx_1_6.dll`; registers the plugin with `OrtEnv.RegisterExecutionProviderLibrary`; enumerates `OrtEnv.GetEpDevices()`; and appends the `NvTensorRTRTXExecutionProvider` GPU device explicitly. It does not call Windows ML `ExecutionProvider.TryRegister`, `EnsureAndRegisterCertifiedAsync`, or `SetEpSelectionPolicy` for TRT RTX.
