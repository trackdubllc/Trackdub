"""Evaluate quantized ONNX vs PyTorch Qwen2.5-VL-3B-Instruct on AI2D (diagram understanding).
...
Usage:
    ...
    python eval.py --pytorch_model Qwen/Qwen2.5-VL-3B-Instruct

    # Larger sample
    python eval.py --num_samples 200 --pytorch_model Qwen/Qwen2.5-VL-3B-Instruct
"""
...
def run_onnx(model, processor, tokenizer, pil_image: Image.Image, messages_json: str) -> str:
    ...
    try:
        ...
        # max_length is total tokens (prompt + generated). Prompts can be up to ~1800 tokens
        # with larger images after the smart_resize fix (more patches = more vision tokens).
        # We only need 1-2 generated tokens (a digit), so 2000 provides ample headroom.
...
def main():
    parser = argparse.ArgumentParser(description="Eval ONNX (quantized) vs PyTorch Qwen2.5-VL on AI2D")
    ...
    parser.add_argument("--pytorch_model", default=None,
                        help="HuggingFace model ID for PyTorch comparison, e.g. Qwen/Qwen2.5-VL-3B-Instruct")
