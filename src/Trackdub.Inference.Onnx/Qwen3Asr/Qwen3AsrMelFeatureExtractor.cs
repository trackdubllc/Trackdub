...
/// <summary>
/// 16 kHz log-mel spectrogram for Qwen3-ASR (128 bins, Whisper-compatible preprocessing).
/// </summary>
internal sealed class Qwen3AsrMelFeatureExtractor
{
    ...
    public DenseTensor<float> Extract(ReadOnlySpan<float> inputSamples)
    {
        ...
        try
        {
            ...
            float[,] powerSpectrum = ComputePowerSpectrum(paddedForStft, frameCount);
            float[,] melSpectrum = ApplyMelFilters(powerSpectrum, frameCount);
            NormalizeLogMel(melSpectrum, frameCount);
    ...
    private float[,] ComputePowerSpectrum(ReadOnlySpan<float> paddedSamples, int frameCount)
    {
    ...
    private float[,] ApplyMelFilters(float[,] powerSpectrum, int frameCount)
    {
    ...
    private static void NormalizeLogMel(float[,] melSpectrum, int frameCount)
    {
